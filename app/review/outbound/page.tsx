...
/*  MAIN COMPONENT                                                            */
/* -------------------------------------------------------------------------- */
function OutboundReviewPageContent() {
  ...
  useEffect(() => {
    if (isLoaded && isSignedIn && userId) {
      const fetchInstructions = async () => {
        setIsLoading(true);
        try {
          const res = await fetch("/api/instructions?type=outbound");
          if (!res.ok) throw new Error(`Failed to fetch: ${res.status}`);
          const data = await res.json();
          console.log("Received instructions data:", data);
            ...
            <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
              <div className="px-4 lg:px-6">
                <Card className="mx-auto max-w-4xl purple-card">
                  <CardHeader>
                    <CardTitle className="text-foreground">Outbound Call Prompts</CardTitle>
                    <CardDescription>
