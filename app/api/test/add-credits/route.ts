...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log(`TEST: Adding ${creditAmount} credits for user ${userId} with email ${userEmail}`);
    ...
    if (!foundUser.length) {
      console.log(`TEST: User ${userId} not found in users table, trying to find by email ${userEmail}`);
      ...
      if (foundUser.length > 0) {
        console.log(`TEST: Found user by email: ${foundUser[0].id}`);
      }
    }
    ...
    console.log(`TEST: Using user ID ${actualUserId} for credit operations`);
    ...
    if (existingCompanyData.length > 0) {
      ...
      console.log(`TEST: Updated user ${actualUserId} credits from ${currentCredits} to ${newCredits}`);
      ...
      console.log(`TEST: Created new company data for user ${actualUserId} with ${creditAmount} credits`);
    ...
    console.error('TEST: Error updating credits for user:', error);
    return NextResponse.json(
      { error: 'Failed to update credits', details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    );
  }
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.log(`TEST: Looking for credits for user ${userId} with email ${userEmail}`);
    ...
    // Use the found user ID (either from Clerk or from email lookup)
    const actualUserId = foundUser[0].id;
    console.log(`TEST: Using user ID ${actualUserId} for credit lookup`);
    ...
    console.error('TEST: Error retrieving credits for user:', error);
    return NextResponse.json(
      { error: 'Failed to retrieve credits', details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    );
  }
}
