// app/api/external-posts/alert/route.ts
import { NextRequest, NextResponse } from 'next/server';
import { getValidAccessToken } from '@/lib/oauth-utils';

export async function POST(request: NextRequest) {
  try {
    console.log('Alert post creation API called');
    
    // Parse JSON body
    const body = await request.json();
    
    // Extract data for ALERT posts
    const {
      title,
      imageUrl,
      videoUrl,
    ...
    // Validate required fields for ALERT posts
    if (!title) {
      return NextResponse.json(
        { error: 'Title is required for alert posts' },
        { status: 400 }
      );
    }
    ...
    console.log('Access token found, processing alert post creation...');
    ...
    console.log('Parent path for alert post creation:', parentPath);

    // Prepare the post data for ALERT type
    const postData: any = {
      languageCode: 'en',
      summary: title,
      topicType: 'ALERT',
      alertType: 'COVID_19' // Default alert type
    };
    ...
    if (imageUrl && imageUrl.trim() !== '') {
      ...
      console.log('Added image to alert post:', imageUrl.trim());
    } else if (videoUrl && videoUrl.trim() !== '') {
      postData.media = [{
        mediaFormat: 'VIDEO',
        sourceUrl: videoUrl.trim()
      }];
      console.log('Added video to alert post:', videoUrl.trim());
    }

    console.log('Final alert post data:', JSON.stringify(postData, null, 2));

    // Create the post using Google My Business v4 API
    const postUrl = `https://mybusiness.googleapis.com/v4/${parentPath}/localPosts`;
    
    console.log('Creating alert post at URL:', postUrl);
    ...
    console.log('Alert post creation response status:', postResponse.status);
    ...
    if (!postResponse.ok) {
      ...
      return NextResponse.json(
        { 
          error: 'Failed to create alert post',
          details: errorText,
    ...
    console.log('Alert post created successfully:', postResult);

    return NextResponse.json({
      success: true,
      message: 'Alert post created successfully',
      post: postResult
    });

  } catch (error) {
    console.error('Error creating alert post:', error);
    return NextResponse.json(
      { 
