...
export async function GET(req: NextRequest) {
  ...
  try {
    // Query the call_history table to get the call count per day for the selected period
    ...
    // Build a mapping from the full date string (YYYY-MM-DD) to call count
    const queryData: Record<string, number> = {};
    result.rows.forEach((row: any) => {
      // If row.day is not a Date object, create one from the string
      const dayDate = row.day instanceof Date ? row.day : new Date(row.day);
      const dayStr = dayDate.toISOString().split('T')[0];
      queryData[dayStr] = parseInt(row.call_count, 10);
    });

    // Generate an array of dates for every day in the range [startDateValue, endDateValue] inclusive
    const finalData: { date: string; calls: number }[] = [];
